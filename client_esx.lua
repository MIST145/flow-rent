...
AddEventHandler('flow-rent:enoughMoney', function()
    ESX.ShowNotification('You don\'t have enough money', 'error', 5000)
end)
...
RegisterNUICallback("rentCar", function(carNames)
    ...
    newPrice = tonumber(price:sub(1, #price-1))
    ...
    if counter ~= 0 then
        ...
        vehicle = CreateVehicle(vehiclename, pos, heading, true)
        ...
        ESX.ShowNotification('Please choose renting time and car', 'error', 5000)
    end
    ...
    if veh ~= 0 then
        ...
        SetEntityAsMissionEntity(veh, true, true)
        DeleteVehicle(veh)
        ESX.ShowNotification('Your car rental period has expired', 'error', 5000)
    end
end)
...
RegisterNUICallback("returnVehicle", function()
    ...
    if vehicle then
        ...
        SetDisplay(false)
        ESX.ShowNotification('You have successfully delivered the vehicle', 'success', 5000)
        vehicle = false
    end
end)
...
Citizen.CreateThread(function()
    ...
    local npc = CreatePed(4, GetHashKey(Config.NpcModel), npcVector1.x, npcVector1.y, npcVector1.z, npcVector1.heading, false, true)
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if inZone and not alreadyEnteredZone then
                ...
                ESX.ShowHelpNotification('Please Press ~INPUT_CONTEXT~')
            end
